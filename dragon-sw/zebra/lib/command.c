...
static void (*config_end_callback_func)(struct vty *vty) = NULL;  
...
void set_config_end_call_back_func(void (*func)(struct vty*))
{
	config_end_callback_func = func;
}
...
/* Initialize command interface for NSF DRAGON. Install basic nodes and commands. */
void
dragon_cmd_init ( int (*dragon_config_write_func)(struct vty *vty))
{
  /* Allocate initial top vector of commands. */
  cmdvec = vector_init (VECTOR_MIN_SIZE);
  ...
  /* Install top nodes. */
  install_node (&config_node, dragon_config_write_func);
  install_node (&auth_node, NULL);
  install_node (&view_node, NULL);
