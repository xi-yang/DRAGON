...
int is_ospf_te_lsa(struct lsa_header *lsah)
{
   ...
   if((lsah->type == OSPF_OPAQUE_AREA_LSA) &&
   	GET_OPAQUE_TYPE(ntohl(lsah->id.s_addr)) == OPAQUE_TYPE_TE_AREA_LSA)
   {
       ret = 1;
       goto out;
   }
   if((lsah->type == OSPF_OPAQUE_LINK_LSA) &&
   	GET_OPAQUE_TYPE(ntohl(lsah->id.s_addr)) == OPAQUE_TYPE_TE_LINKLOCAL_LSA)
   {
       ret = 1;
       goto out;
   }
  ...
  */
  ...
  switch (lsa->data->type)
	...
	if (is_ospf_te_lsa(lsa->data))
	  {
	   /* lsa->te_lsa_type = *((char*)lsa->data+21); */
	    ospf_te_lsa_install(lsa, oi);
	  }	
      break;
#endif /* HAVE_OPAQUE_LSA */
