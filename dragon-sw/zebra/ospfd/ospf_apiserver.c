...
ospf_apiserver_init (void)
{
  ...
  fd = ospf_apiserver_serv_sock_family (OSPF_API_SYNC_PORT, AF_INET);
...
ospf_apiserver_new (int fd_sync, int fd_async)
{
  ...
  regtype->lsa_type = 10;
  regtype->opaque_type = 1;
  listnode_add(new->opaque_types, regtype);
  */
...
ospf_apiserver_accept (struct thread *thread)
{
  ...
  if (ntohs (peer_async.sin_port) == OSPF_API_SYNC_PORT)
    {
  ...
  /* re-regitered LSA-Opaque Type 10-1 if unregistered*/
#if 0
  registered = 0;  
  if ((funclist = ospf_get_opaque_funclist (OSPF_OPAQUE_AREA_LSA)) != NULL)
    {
      listnode node;
 ...
 if (!registered)
     ospf_register_opaque_functab(OSPF_OPAQUE_AREA_LSA, 1, 
				  ospf_apiserver_new_if,
				  ospf_apiserver_del_if,
				  ospf_apiserver_ism_change,
				  ospf_apiserver_nsm_change,
...
/*
 *   hacked $$$$ copy registered lsa and opaque types into the list: apiserver->opaque_types
 */
void
ospf_apiserver_import_registered_opaque_types(struct ospf_apiserver *apiserv,
                                     u_char lsa_type)
{
  list funclist;
  listnode node;
  ...
  funclist = ospf_get_opaque_funclist(lsa_type);
  assert(funclist);
  assert(apiserv->opaque_types);
      ...
      if ((functab = getdata (node)) != NULL)
        {
          ...
          regtype->opaque_type = functab->opaque_type;
          
          for (typenode = listhead(apiserv->opaque_types); typenode; nextnode(typenode))
            {
              if (regtype->lsa_type == ((struct registered_opaque_type *)getdata(typenode))->lsa_type 
                  && regtype->opaque_type == ((struct registered_opaque_type *)getdata(typenode))->opaque_type)
                break;
            }
         if (!typenode)
...
ospf_apiserver_lsa_refresher (struct ospf_lsa *lsa)
{
  ...
  if (is_ospf_te_lsa(new->data) && (!new->tepara_ptr))
  {
  	ospf_te_lsa_parse(new);
  }
				      ...
				      struct msg *msg)
{
  ...
  */  
  adv_router = dmsg->adv_router;
  old = ospf_lsa_lookup (area, dmsg->lsa_type, id, adv_router);
...
ospf_apiserver_handle_neighbor_count_request (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  neighbor_count_reply = XMALLOC(MTYPE_TMP, sizeof(struct msg_neighbor_count));
  neighbor_count_reply->count = 0;
  neighbor_count_reply->status = 0; /* status currently not defined*/
  ...
  if (ospf && ospf->oiflist)
    {
      for (node = listhead (ospf->oiflist); node; nextnode (node))
        {
          oi = getdata(node);
          neighbor_count_reply->count = ospf_nbr_count (oi, NSM_Full);
        }
    }

  rmsg = msg_new(MSG_NEIGHBOR_COUNT, neighbor_count_reply, 
            seqnum, sizeof(struct msg_neighbor_count));
					 ...
					 struct msg *msg)
{
  ...
  struct msg_narb_cspf_reply* cspf_reply;
  struct msg *rmsg;
  ...
  cspf_req = (struct msg_narb_cspf_request*)STREAM_DATA(msg->s);

  /*obtaining instances of ospf and area*/
  ospf = ospf_lookup();
  assert (ospf);

  area = ospf_area_lookup_by_area_id (ospf, cspf_req->area_id);
  if (area)
    {
      /*cspf routing calculation on demand*/
      explicit_path=ospf_cspf_calculate (area, cspf_req->app_req_data.src, 
            cspf_req->app_req_data.dest, cspf_req->app_req_data.switching_type);
    }

  /*if no route found, reply with an error code*/
  if (!explicit_path)
    {
      length = sizeof(struct msg_narb_cspf_reply) + 4;
      cspf_reply = XMALLOC(MTYPE_TMP, length);

      cspf_reply->narb_apiserv_id = cspf_req->narb_apiserv_id;
      cspf_reply->app_seqnum = cspf_req->app_seqnum;
      cspf_reply->src = cspf_req->app_req_data.src;
      cspf_reply->dest = cspf_req->app_req_data.dest;
      cspf_reply->tlv.type  = htons (TLV_TYPE_NARB_ERROR_CODE);
      cspf_reply->tlv.length  = htons (4);

      *(u_int32_t *)((char *)cspf_reply + sizeof(struct msg_narb_cspf_reply)) = htonl(CSPF_ERROR_NO_ROUTE);
      rmsg = msg_new(MSG_NARB_CSPF_REPLY, cspf_reply, ntohl(msg->hdr.msgseq), length);
  ...
  /*other wise reply with ERO*/
  length = sizeof(struct msg_narb_cspf_reply) + sizeof(struct in_addr) * explicit_path->count;
  cspf_reply = XMALLOC(MTYPE_TMP, length);
  ...
  cspf_reply->src = cspf_req->app_req_data.src;
  cspf_reply->dest = cspf_req->app_req_data.dest;
  cspf_reply->tlv.type  = htons (TLV_TYPE_NARB_ERO);;
  cspf_reply->tlv.length  = htons (sizeof(struct in_addr) * explicit_path->count);

  router_ip = (struct in_addr *)((char *)cspf_reply + sizeof(struct msg_narb_cspf_reply));
  for (node = explicit_path->head; node; nextnode (node)) 
    {
      *(router_ip++) = *(struct in_addr*) node->data;
      XFREE(MTYPE_TMP, node->data);
    }

 /*free explicit_path immediately after ERO information is extracted*/
 if (explicit_path)
