...
static const char *
ospf_opaque_type_name (u_char lsa_type, u_char opaque_type)
{
  	...
  	case OSPF_OPAQUE_AREA_LSA:
		  switch (opaque_type)
		    ...
		    case OPAQUE_TYPE_WILDCARD: /* This is a special assignment! */
		      name = "Opaque Area LSA (type-10) Wildcard";
		      break;
		    case OPAQUE_TYPE_TE_AREA_LSA:
		      name = "Opaque Area LSA (type-10) TE-LSA";
		      break;
		    case OPAQUE_TYPE_SYCAMORE_OPTICAL_TOPOLOGY_DESC:
		      name = "Opaque Area LSA (type-10) Sycamore optical topology description";
		      break;
		    case OPAQUE_TYPE_GRACE_LSA:
		      name = "Opaque Area LSA (type-10) Grace-LSA";
		      break;
		    default:
		      if (OPAQUE_TYPE_RANGE_UNASSIGNED (opaque_type))
		        name = "Opaque Area LSA (type-10) Unassigned";
		      else if (OPAQUE_TYPE_RANGE_RESERVED (opaque_type))
		        name = "Opaque Area LSA (type-10) Private/Experimental";
		      break;
		    }
  		break;
  	case OSPF_OPAQUE_LINK_LSA:
  		name = "Opaque Link LSA (type-9) GMPLS-TE-Link-Local LSA";
  		break;
  	default:
  		break;
  }
  ...
  if (vty != NULL)
    {
      vty_out (vty, "  Opaque-Type %u (%s)%s", opaque_type,
	       ospf_opaque_type_name (lsa_type, opaque_type), VTY_NEWLINE);
  ...
  else
    {
      zlog_info ("    Opaque-Type %u (%s)", opaque_type,
		 ospf_opaque_type_name (lsa_type, opaque_type));
  ...
  ||   functab->lsa_refresher == NULL)
    {
       ...
       */
       /* However, we shall not flush TE-LSA in this case because TE-LSA refresh functions
        * are not registered.
        */
...
ospf_opaque_lsa_flush_schedule (struct ospf_lsa *lsa0)
{
  ...
  /*oipi->lsa = NULL;*/ /* Aihua: Removed for bug fixes */
  free_opaque_info_per_id ((void *) oipi);

  /* Xi: Avoid crash due to wrong lsa data*/
  if (!lsa->data)
    {
