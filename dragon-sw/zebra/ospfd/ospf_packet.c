...
/* Patch from Quagga 0.98 
    write over-sized OSPF packets
*/
void
  ...
  for (; size >= OSPF_LSA_HEADER_SIZE && count > 0;
      ...
      if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA)
	{
      ...
      switch (lsah->type)
        ...
        /* We are not parsing TE parameters here. 
        ...
        if (is_ospf_te_lsa(lsah))
        {
        	...
        	{
	            if (IS_DEBUG_OSPF_EVENT)
		              zlog_info("LSA[Type%d:%s]: %p new TE LSA was not parsed successfully",
        	...
        	{
		     if (IS_DEBUG_OSPF_EVENT)
	  			zlog_info("TE-LSA[Type%d:%d]: %p new TE LSA created with Link State Update",
	  	  				lsa->data->type, GET_OPAQUE_ID(ntohl(lsa->data->id.s_addr)), lsa->data);
	     ...
	     struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  opaque_acks->del = ospf_lsa_discard;
#endif /* HAVE_OPAQUE_LSA */
