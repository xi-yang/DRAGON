				  ...
				  struct ospf_lsa *new, struct ospf_area *area)
{
  ...
  switch (new->data->type)
		  ...
		  !IPV4_ADDR_SAME (&oi->address->u.prefix4, &DR (oi)))
		{
		  ...
		  return;
              ...
              if (new->data->type == OSPF_OPAQUE_LINK_LSA)
                {
		    if (is_ospf_te_lsa(new->data) && new->oi == oi && INTERFACE_GMPLS_ENABLED(oi))
			    ospf_te_linklocal_lsa_refresh(new);
		    else
    ...
    case OSPF_OPAQUE_AREA_LSA:
      if (is_ospf_te_lsa(new->data) )
      {
      		if (new->te_lsa_type == NOT_TE_LSA || (!new->tepara_ptr))
      			new = ospf_te_lsa_parse(new);
      		if (new->te_lsa_type == NOT_TE_LSA) /* This is really not a TE-LSA or this LSA is badly formated */
      			ospf_opaque_lsa_refresh(new);
      		else
      		{
		       for (node = listhead (ospf->oiflist); node; nextnode (node))
			...
			if ((oi = getdata (node)) != NULL)
			{
		      	      if (new->oi == oi && GET_OPAQUE_ID(ntohl(new->data->id.s_addr)) == oi->te_para.instance)
		      	      	{
		      	      		if (new->te_lsa_type == ROUTER_ID_TE_LSA)
				      	      ospf_te_area_lsa_rtid_refresh(new);
		      	      		else if (new->te_lsa_type == LINK_TE_LSA)
		      	      			ospf_te_area_lsa_link_refresh(new);
			      	      break;
		      	      	}
			}
      		}
