...
/* possible values: NOT_TE_LSA(0)/ROUTER_ID_TE_LSA(1)/LINK_TE_LSA(2)/LINKLOCAL_TE_LSA(3) 
    This is defined in ospf_te_lsa.h 
*/
  u_char te_lsa_type;  

 /* Pointer to TE tlvs and sub-tlvs. 
 If this is a TE-LSA, then this pointer MUST not be NULL. */
  struct te_lsa_para_ptr *tepara_ptr; 

  /* Parent TE-LSDB */
  struct ospf_te_lsdb *te_lsdb;
...
#ifdef HAVE_OPAQUE_LSA
int is_ospf_te_lsa(struct lsa_header *lsah);
#endif
