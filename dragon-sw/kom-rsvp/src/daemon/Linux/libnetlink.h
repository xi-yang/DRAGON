...
class RTNetlink
{
  public:
	...
	static int rtnl_dump_request(rtnl_handle *rth, int type, void *req, int len);
	...
	static int rtnl_listen(struct rtnl_handle *, int (*handler)(struct sockaddr_nl *,struct nlmsghdr *n, void *),
	                       void *jarg);
	static int rtnl_ask(rtnl_handle *rtnl, struct iovec *iov, int iovlen,
	...
	static int rtnl_tell_iov(rtnl_handle *rth, struct iovec *iov, int iovlen);
