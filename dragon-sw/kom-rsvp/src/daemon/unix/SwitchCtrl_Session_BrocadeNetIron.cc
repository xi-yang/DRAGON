...
{
    CLI_Session::disengage("end\r\exit\rexit\r");
...
bool SwitchCtrl_Session_BrocadeNetIron::pipeAlive()
{
    ...
    int n;
    pipe_broken = false;
    DIE_IF_NEGATIVE(n= writeShell( "\r", 5)) ;
...
bool SwitchCtrl_Session_BrocadeNetIron::preAction()
{
    if (!active)
        return false;
    int n;
    DIE_IF_NEGATIVE(n= writeShell( "\r", 10)) ;
    DIE_IF_NEGATIVE(n= readShell( ">", "#", true, 1, 20)) ;
    if (n == 1)
    {
        DIE_IF_NEGATIVE(n= writeShell( "enable\r", 10));
    ...
    DIE_IF_NEGATIVE(writeShell("configure terminal\r", 10));
...
bool SwitchCtrl_Session_BrocadeNetIron::postAction()
{
    if (fdout < 0 || fdin < 0)
        return false;
    DIE_IF_NEGATIVE(writeShell("end\r", 10));
...
bool SwitchCtrl_Session_BrocadeNetIron::movePortToVLANAsUntagged(uint32 portID, uint32 vlanID)
{
    ...
    if (old_vlan > 1) { //Remove untagged port from old VLAN
        ret &= deleteVLANPort_ShellScript(portID, old_vlan);
    }
...
bool SwitchCtrl_Session_BrocadeNetIron::removePortFromVLAN(uint32 portID, uint32 vlanID)
{
    ...
    if (vlanID>=MIN_VLAN && vlanID<=MAX_VLAN){
        ...
        if (vpmAll) {
            ResetPortBit(vpmAll->portbits, bit);
            ret &= this->deleteVLANPort_ShellScript(portID, vlanID);
        }
    } else {
...
bool SwitchCtrl_Session_BrocadeNetIron::deleteVLANPort_ShellScript(uint32 portID, uint32 vlanID)
{
...
bool SwitchCtrl_Session_BrocadeNetIron::hook_createVLAN(const uint32 vlanID)
{
    ...
    DIE_IF_NEGATIVE(n = writeShell( "no spanning-tree\r", 10));
    DIE_IF_NEGATIVE(n= readShell( "#", NULL, true, 1, 20));

    DIE_IF_NEGATIVE(n = writeShell( "no rstp\r", 10));
    DIE_IF_NEGATIVE(n= readShell( "#", NULL, true, 1, 20));
...
bool SwitchCtrl_Session_BrocadeNetIron::hasVlanRateLimit(uint32 vlan, bool is_input)
{
    ...
    DIE_IF_NEGATIVE(n = writeShell( "show rate-limit\r", 10));
