...
public:
	...
	inline double rand( const double& n );     // real number in [0,n]
	inline double randExc();                   // real number in [0,1)
	inline double randExc( const double& n );  // real number in [0,n)
	inline uint32 randInt();                        // integer in [0,2^32-1]
	inline uint32 randInt( const uint32& n );       // integer in [0,n]
	double operator()() { return rand(); }   // same as rand()
	
	// Re-seeding functions with same behavior as initializers
	inline void seed( uint32 oneSeed );
	inline void seed( uint32 *const bigSeed );
	inline void seed();
	
	// Saving and loading generator state
	inline void save( uint32* saveArray ) const;  // to array of size N+1
	inline void load( uint32 *const loadArray );  // from such array
	friend inline ostream& operator<<( ostream& os, const MTRand& mtrand );
	friend inline istream& operator>>( istream& is, MTRand& mtrand );
...
inline void MTRand::seed()
{
	...
	*/
	...
	seed( hash( time((time_t*)NULL), clock() ) );
}
